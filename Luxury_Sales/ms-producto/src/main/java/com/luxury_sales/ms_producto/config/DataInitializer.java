...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        if (productoRepository.count() == 0) {
            // Cambiamos los productos de lujo por repuestos de autos con precios reales
            productoRepository.save(new Producto("Pastillas de Freno Delanteras", 45.990, 20));
            productoRepository.save(new Producto("Amortiguador Hidráulico", 125.670, 15));
            productoRepository.save(new Producto("Filtro de Aceite Sintético", 12.000, 50));
            
            System.out.println(">>> ms-producto: 3 repuestos de autos insertados como datos de prueba.");
        }
    }
}